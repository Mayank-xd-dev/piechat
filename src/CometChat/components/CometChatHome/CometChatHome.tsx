...
function CometChatHome({ defaultUser, defaultGroup }: CometChatHomeProps) {
  ...
  useEffect(() => {
    ...
    const ccGroupMemberBanned = CometChatGroupEvents.ccGroupMemberBanned.subscribe((_item: IGroupMemberKickedBanned) => {
      toastTextRef.current = getLocalizedString('member_banned');
      setShowToast(true);
    });
    const ccGroupMemberKicked = CometChatGroupEvents.ccGroupMemberKicked.subscribe((_item: IGroupMemberKickedBanned) => {
  ...
  const SideComponentGroup = React.memo((props: { group: CometChat.Group }) => {
    ...
    useEffect(() => {
      CometChat.addGroupListener(
        ...
        new CometChat.GroupListener({
          onGroupMemberScopeChanged: (_message: CometChat.Action, changedUser: CometChat.GroupMember, _newScope: CometChat.GroupMemberScope, _oldScope: CometChat.GroupMemberScope, changedGroup: CometChat.Group) => {
          ...
          onGroupMemberKicked: (_message: CometChat.Action, _kickedBy: CometChat.User) => {
            setMemberCount(group.getMembersCount());
            setGroup(group);
          },
          onGroupMemberBanned: (_message: CometChat.Action, _kickedBy: CometChat.User) => {
            setMemberCount(group.getMembersCount());
            setGroup(group);
          },
          onGroupMemberAdded: (_message: CometChat.BaseMessage, _userAdded: CometChat.User, _userAddedBy: CometChat.User, userAddedIn: CometChat.Group) => {
            setMemberCount(userAddedIn.getMembersCount());
            setGroup(userAddedIn);
          },
          onGroupMemberLeft: (_message: CometChat.BaseMessage, _leavingUser: CometChat.GroupMember, group: CometChat.Group) => {
            setMemberCount(group.getMembersCount());
            setGroup(group);
          },
          onGroupMemberJoined: (_message: CometChat.BaseMessage, _joinedUser: CometChat.GroupMember, joinedGroup: CometChat.Group) => {
    ...
    }, [group, memberCount, chatFeatures?.groupManagement]);
